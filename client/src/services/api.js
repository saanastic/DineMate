...
const createDemoOrder = (payload) => {
  ...
  const order = {
    ...
    table_label:
      payload.table_label ||
      (payload.table_id ? `Table ${payload.table_id}` : null),
    status: "placed",
    payment_method: payload.payment_method || "cash",
    payment_status:
      payload.payment_method === "online" ? "pending" : "paid",
...
export const dashboardService = {
  ...
  getKitchenQueue: async () => {
    try {
      ...
      return orders.filter((o) =>
        ["placed", "confirmed", "preparing", "ready", "served"].includes(
          o.status
        )
      );
    }
  ...
  getBilling: async () => {
    try {
      ...
      return {
        invoices: orders.slice(0, 20).map((o) => ({
          id: o.id,
          total: o.total,
  ...
  getAiAssistant: async () => {
    try {
      ...
      return {
        summary:
          "DineMate AI is ready — start a conversation to get insights based on demo data.",
      };
    }
  },
};
  ...
  chat: async (payload) => {
    try {
      ...
      if (q.includes("sales") || q.includes("revenue")) {
        ...
        return {
          reply: `Today's demo revenue is ₹${Math.round(
            revenue
          )} across ${todayOrders.length} orders.`,
        };
      }
      ...
      if (q.includes("top") && q.includes("dish")) {
        ...
        orders.forEach((o) =>
          (o.items || []).forEach((it) => {
            counts[it.item_name] =
              (counts[it.item_name] || 0) + (it.quantity || 1);
          })
        );
        ...
        return {
          reply: top
            ? `Top demo dish: ${top[0]} (${top[1]} sold)`
            : "No sales yet in demo.",
        };
      }

      return {
        reply:
          "I can help with demo data — ask about sales, top dishes, kitchen status, or inventory.",
      };
    }
  },
};
      ...
      return [
        {
          ...
          description:
            "Creamy tomato sauce with soft paneer cubes, served with butter naan.",
          price: 249,
          bestseller: true,
          veg: true,
          spice_level: 2,
        ...
        },
        {
          ...
          category: "Main Course",
          description:
            "Aromatic basmati rice layered with spiced chicken and fried onions.",
          price: 349,
          bestseller: true,
          veg: false,
          spice_level: 3,
        ...
        },
        {
          ...
          description:
            "Crispy rice crepe filled with spiced potato masala, served with chutney.",
          price: 129,
          bestseller: false,
          veg: true,
          spice_level: 1,
